...
def login():
    ...
    return flask.render_template('register.html', invalid=True) #return an invalid flag when passwords do not match
...
def register():
    ...
    if query.fetch().num_results > 0: #if there is any result then the username already exists
        return flask.render_template('register.html', invalid=True) #return an invalid flag when user already exists
